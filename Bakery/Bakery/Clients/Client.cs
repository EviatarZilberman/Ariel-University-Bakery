...
namespace Bakery.Clients
{
    class Client // Class atributes.
    {
        ...
        private double PurchaseSummarry;
        private static int idGen = 1; // Id for every client for the logs updates.
        private int id;
        private bool threadOrManual;
        ...
        public Client(string firstName, string lastName, int shoppingListLength)
        {
            ...
            {
            this.threadOrManual = true;
            }
            else
            {
                this.threadOrManual = false;
            }
        }
        ...
        public bool ThreadOrManual
        {
            get { return threadOrManual; }
            set { threadOrManual = value; }
        }
                                                ...
                                                // the menu will be presented.
        {
            ...
            try // Try to let the user Choose from the numbers of the products.
            {
                for (int i = 0; i < this.list.Length; i++)
                {
                    ...
                    if (this.List[i].DemandOfProducts <= bakery.ProductsInBakery[productUserChoose].AmountInBakery) // If there are more products in the bakery than the client wants.
                    {
                        this.List[i].BoughtProducts = amountUserChoose;
                        bakery.ProductsInBakery[productUserChoose].AmountInBakery-= this.List[i].DemandOfProducts;
                    }
                    else
                    {
                        ...
                        bakery.ProductsInBakery[productUserChoose].AmountInBakery = 0;
                    }
                    totalSum += this.List[i].BoughtProducts * bakery.ProductsInBakery[productUserChoose].Price;                    
        ...
        public void FileComplaint(TheBakery bakery) // Method that randomally files complaints on random employees.
        {
            ...
            if (bakery.Employees[sComplaint].GetType() == typeof(BakeryManager)) // Find the type of the employee for the complain and sets it into a string.
            {
                complainedEmployee = "manager";
            } else if(bakery.Employees[sEmploee].GetType() == typeof(Baker))
            {
                complainedEmployee = "baker";

            } else if (bakery.Employees[sComplaint].GetType() == typeof(Cashier))
            {
